...
{
    /// <inheritdoc />
    public partial class FixInventoryReport : Migration
    {
        /// <inheritdoc />
        ...
        WITH InwardCTE AS (
                    SELECT 
					it.ProductId, 
					SUM(it.ItemQuantity) AS TotalInwardQty
             ...
             SELECT
               ...
               SUM(
                ...
                THEN
                    CASE
                        WHEN parent.IsInStock = 0 THEN 0
                        ELSE 1
                    END
           ...
           INNER JOIN dbo.InwardBarcodeItems ibt
               ON od.BarcodeNo = ibt.BarcodeNo  AND ibt.IsDeleted = 0
           INNER JOIN dbo.InwardItems it
               ON it.Id = ibt.InwardItemId
           LEFT JOIN dbo.InwardBarcodeItems parent
               ON parent.Id = ibt.ParentId    AND parent.IsDeleted = 0
           WHERE 
			  od.IsDeleted = 0
			  AND ibt.IsDeleted = 0
			  AND (parent.IsDeleted = 0 OR parent.Id IS NULL)
			  AND ibt.IsInStock = 0
           GROUP BY it.ProductId
		),
                       ReturnCTE AS (
			         SELECT 
   			         sri.ProductId, 
	       		         SUM(sri.ReturnQuantity) AS ReturnQty
			         FROM dbo.SaleReturnItems AS sri
				 WHERE sri.IsDeleted = 0
				 AND sri.IsTakeInStock = 1
			     ...
			     p.Name as ProductName,
                             ...
                             ISNULL(i.TotalInwardQty, 0) - (ISNULL(s.IssuedQty, 0) ) AS GoodStock, 
                             p.CategoryId, 
                             p.MakeCompanyId, 
                             p.ColourId
