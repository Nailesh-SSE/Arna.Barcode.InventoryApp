...
{
    /// <inheritdoc />
    public partial class FixBarcodeTrackingReport : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"   ALTER PROCEDURE [dbo].[sp_BarcodeTracking]
    @BarcodeNo NVARCHAR(50)
            ...
            BEGIN
             ...
             -- INWARD 
               ...
               SELECT 
                    ...
                    CASE 
                    WHEN sri.UnitId = 2 THEN   -- BOX return
                        CASE 
                            WHEN ibt.ParentId <> 0 THEN ibt.BarcodeNo   -- scanned ITEM
                            ELSE NULL                                   -- scanned BOX → no item
                        END
                    ELSE sri.BarCodeNo   -- PCS return
                END AS Item_Barcode,
                    CASE 
            ...
            -- Step 1: Find parent box
            LEFT JOIN InwardBarcodeItems box
                ON ibt.ParentId = box.Id
            
            -- Step 2: Match return using BOX barcode
            INNER JOIN SaleReturnItems sri
                 ON 
                (
                    --  Case 1: direct item match (highest priority)
                    sri.BarCodeNo = ibt.BarcodeNo
            
                    OR
            
                    -- Case 2: fallback to BOX only if item match doesn't exist
                    (
                        ibt.ParentId <> 0 
